...
The worker has no HTTP server and does not import `shared/metrics`. "Worker behaviour" metrics (agent execution time, prepare time, exit code, ...) are derived by the app's result listener from data the worker reports back in `JobResult` / `StatusReport`. v3 added `JobResult.ExitCode` (sentinel `-1` for "no captured code") for exactly this — see Decision #7 below. Giving the worker its own `/metrics` endpoint was considered and rejected: it's a larger change than the gap warrants, and it cuts against the product positioning (a structuring tool, not a diagnosis platform).

### 7. `JobResult.ExitCode` uses a `-1` sentinel and is not `omitempty`

`int`'s zero value `0` collides with a successful exit, so a sentinel is needed to mean "no captured code" (pre-runner failures, cancellation before exec, and — see below — any signal-terminated run). We use `-1`. The field is deliberately **not** `omitempty` — `omitempty` would drop the `-1` on the wire and resurrect the ambiguity. The app gates observation on `ExitCode >= 0`, so `-1` never reaches `agentdock_agent_exit_code_total`. Every `JobResult` construction site — in `worker/` *and* in `shared/queue/watchdog.go` (timeout kill, cancel fallback) and `app/app.go` (attachment-prepare failure) — sets `ExitCode: -1` when no agent process produced a code; missing one would inject a phantom `exit_code="0"` sample.

**What `agent_exit_code_total` does and does not see.** It records only codes the agent process *chose for itself* (`0`, `1`, `2`, …). It does **not** see `124`/`137`-style signal codes: Go's `(*exec.ExitError).ExitCode()` returns `-1` (not the shell's `128+signal`) for a process killed by SIGKILL/SIGTERM, so OOM kills and inactivity/deadline timeouts arrive as `-1` and are skipped — and the watchdog's own job-timeout kill (`shared/queue/watchdog.go`) publishes `ExitCode: -1` for the same reason (no real code to report). For those failure modes, use `agentdock_agent_executions_total{status="timeout"|"error"}` — `operations.md` has the queries. Synthesising `128+signal` in the runner was considered and rejected as out of scope for this gap-closure change (and it'd yield `143`/`137`, never `124` — this project doesn't shell out to `timeout(1)`).

**Fallback chains.** When a channel configures multiple agents and an earlier one fails over to the next, only the *last* agent's exit code reaches `JobResult.ExitCode` (the runner's deferred `OnExit` fires per agent, last write wins). This matches `agent_executions_total{status}`, which likewise records only the final outcome.
...
| `agentdock_agent_exit_code_total{agent, exit_code}` | **added in v3** | `agentdock_agent_exit_code_total{provider, exit_code}` — the code the agent chose for itself (`0`/`1`/`2`/…); signal kills (OOM, timeout) surface as `-1` and are excluded — see Decision #7, and `agentdock_agent_executions_total{status}` for those |
