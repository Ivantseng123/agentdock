...
func marshalAppYAML(cfg *appconfig.Config, path string) ([]byte, error) {
	...
	// OTel tracing endpoint — empty = silent skip (no OTLP traffic, but
	// log trace_id still flows through ctx). Env override:
	// OTEL_EXPORTER_OTLP_ENDPOINT.
	text = insertBefore(text, "tracing:",
...
func marshalWorkerYAML(cfg *workerconfig.Config, path string) ([]byte, error) {
	...
	opencodeComment := "# opencode: (opt-in; Phase 3.2 server-mode rollout — omit to use the legacy spawn mode)\n" +
		...
		"#\n" +
		"# Runtime swap caveat: opencode binary version is checked once at worker boot;\n" +
		"# replacing the binary while a worker is alive does NOT re-fire the check.\n"
	text = opencodeComment + text
...
// stripOpencodeBlock removes the marshaled top-level `opencode:` block
// (plus its indented children + blank trailing lines) from the worker
// yaml. marshalWorkerYAML calls this so the generated file shows only
// a commented-out documentation block at the top — defaults still
// apply at runtime via workerconfig.ApplyDefaults when the block is
// absent.
//
// Implementation: simple state machine over top-level keys. When a
// line starts with "opencode:" we enter skip mode; we continue
// skipping any blank line or any line that starts with whitespace
// (i.e. an indented child of the block). The next non-blank, non-
// indented line ends the block and resumes output.
func stripOpencodeBlock(s string) string {
	lines := strings.Split(s, "\n")
	out := make([]string, 0, len(lines))
	skip := false
	for _, line := range lines {
		if !skip && strings.HasPrefix(line, "opencode:") {
			skip = true
			continue
		}
		if skip {
