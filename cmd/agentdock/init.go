...
func marshalAppYAML(cfg *appconfig.Config, path string) ([]byte, error) {
	...
	// Hint for the queue.store knob added in #123. No value-based override
	// (both mem and redis are valid); operators reading the generated yaml
	// need to know the switch exists and what it does.
	text = insertBefore(text, "    store: ", "    # JobStore backend: mem (default; lost on restart) or redis (persisted; survives app restart — recommended in production; see issue #123)\n")
